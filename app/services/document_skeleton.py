...
def _copy_audit_interval(
    ...
    for entry in audit:
        ...
        if entry.get("source") == "deterministic_markup" and (
            overlap_start != entry_start or overlap_end != entry_end
        ):
            # A later deterministic rewrite can retain only part of an earlier
            # marker (for example, one ``#`` from a generated ``##`` heading).
            # Deterministic provenance is bound to its exact generated bytes,
            # unlike source provenance, which remains bound to the full source
            # artifact while its byte range is clipped.
