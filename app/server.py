    ...
    try:
        ...
        if 'grobid' in selected_methods:
            ...
            if is_extraction_cached(file_hash, 'grobid'):
                ...
                grobid = Grobid(base_url=app.config['GROBID_URL'])
        ...
        if merge_enabled and len(selected_methods) >= 1:
            ...
            cache_key = f"{file_hash}_{'_'.join(sorted(selected_methods))}"
            merged_cache_path = os.path.join(app.config['CACHE_FOLDER'], f'{cache_key}_merged.md')

            llm = LLM(api_key=app.config['ANTHROPIC_API_KEY'], model=app.config['LLM_MODEL'])

            if os.path.exists(merged_cache_path):
                print(f"Using cached merged output: {merged_cache_path}")
                ...
                # keeps specific method combination cached merged output
                with open(merged_cache_path, 'w', encoding='utf-8') as f:
                    f.write(merged_md)
                print(f"Merged output saved to: {merged_cache_path}")

                # updates general cached merged output
...
@app.route('/download/<file_hash>/<method>')
